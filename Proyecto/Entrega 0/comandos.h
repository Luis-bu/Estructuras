...
public:
    ManejadorComandos() {
        ...
        comandos["ayuda_comando"] = {"ayuda_comando", 1, ""}; // Caso especial para ayuda <comando>
    ...
    bool validarComando(const std::string& cmd, const std::vector<std::string>& params) {
        ...
        if (it->second.numParametros != params.size()) {
            std::cout << "Número incorrecto de parámetros para " << cmd << ". Se esperaban " 
                      << it->second.numParametros << ", se recibieron " << params.size() << "." << std::endl;
            return false;
        }
        return true;
    }
